...
init_tmux_mode_indicator() {
	local -r \
		...
		prefix_style=$(indicator_style "$prefix_mode_style_config" "fg=black,bg=blue") \
		copy_style=$(indicator_style "$copy_mode_style_config" "fg=black,bg=orange") \
		sync_style=$(indicator_style "$sync_mode_style_config" "fg=black,bg=red") \
